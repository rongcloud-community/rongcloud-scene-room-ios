...
public protocol VoiceRoomGiftSendViewDelegate: AnyObject {
    func onGiftSendButtonClicked()
    func onGiftCountButtonClicked()
}

final public class VoiceRoomGiftSendView: UIView {
    ...
    var count: Int = 1 {
        didSet {
            ...
            arrowImageView.image = UIImage(assetName: "gift_arrow_up")
        }
    }
    
    public init(_ delegate: VoiceRoomGiftSendViewDelegate) {
    ...
    @objc private func onCountButtonClicked() {
        arrowImageView.image = UIImage(assetName: "gift_arrow_down")
