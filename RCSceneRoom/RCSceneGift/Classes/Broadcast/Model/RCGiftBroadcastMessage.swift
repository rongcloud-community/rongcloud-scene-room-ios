...
extension RCGiftBroadcastMessage {
    static func sendMessageIfNeeded(_ gift: RCChatroomGift, room: RCSceneRoom) {
        guard broadcastGifts.contains(gift.giftId) else { return }
        sendMessage(gift.broadcast(room))
    }
    
    static func sendMessageAllIfNeeded(_ gift: RCChatroomGiftAll, room: RCSceneRoom) {
        guard broadcastGifts.contains(gift.giftId) else { return }
        sendMessage(gift.broadcast(room))
    }
...
extension RCChatroomGift {
    func broadcast(_ room: RCSceneRoom) -> String {
